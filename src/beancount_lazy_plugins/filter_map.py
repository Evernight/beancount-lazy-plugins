...
def filter_map(entries, options_map, config_str=None):
    ...
    for entry in entries:
        if isinstance(entry, Custom) and entry.type == 'filter-map' and entry.values[0].value.strip() == 'preset':
            presets[entry.meta['name']] = entry.meta

    # then form all operations
    operations = []
    for entry in entries:
        if isinstance(entry, Custom) and entry.type == 'filter-map' and entry.values[0].value.strip() == 'apply':
            params = {}
    ...
    for op in operations:
        ...
        if OperationParams.TIME.value in op:
            filters.append(TimeFilter(options_map, FavaOptions(), op[OperationParams.TIME.value]))
        ...
        if OperationParams.ADD_TAGS.value in op:
            op['tagValues'] = op[OperationParams.ADD_TAGS.value].replace('#', '').split(' ')
